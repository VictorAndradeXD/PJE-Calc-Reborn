...
| GoldenGenInssBase.java | geração da base do INSS sobre as verbas por competência (soma das diferenças p/ incidências, comum vs 13º) | dirige `MaquinaDeCalculoDoInss.calcularValorBaseVerbas` via **reflection** (`new MaquinaDeCalculoDoInss(new Inss())`; verbas `Informada` com ocorrências; `verba.getOcorrenciasOptimizerListSearch()`); regime INTEGRAL |
| GoldenGenSeguroDesemprego.java | valor da parcela do seguro-desemprego (duas faixas + piso/teto) | dirige o método privado `MaquinaDeCalculoDeSeguroDesemprego.encontraOValorDoSeguroDesemprego` via **reflection** (lê só campos de faixa/remuneração do `SeguroDesemprego`, dispensa demissão/correção/verbas); alíquotas da tabela real 01/2021. `export_seguro_desemprego.sql` alimenta fixture/sqlite |
| GoldenGenSalarioFamilia.java | seleção de faixa do salário-família (cota por faixa de remuneração; nula acima da 2ª faixa) sobre a tabela real TBTABELASALARIOFAMILIA | carrega `TabelaSalarioFamilia` por JDBC (setters) e chama `getValorSalarioFamiliaParaO(remuneração)`. `export_salario_familia.sql` alimenta a fixture/sqlite |
| GoldenGenFgtsGeracao.java | prescrição do FGTS (STF ARE 709212: 30/5 anos, com transição) e janela sugerida de geração (max(admissão, prescrição) → demissão/término) | `Calculo` com datas + `Fgts` no-arg; lê `getDataPrescricaoFgts()` e `PeriodoDoFgtsValidRule.getPeriodoSugerido(fgts)`. Datas em yyyy-MM-dd |
| GoldenGenPensao.java | pensão alimentícia: bases (verbas com incidência de pensão — diferença corrigida, férias pela parcela gozada; FGTS + multa) e valor devido = total das bases × alíquota | dirige `MaquinaDeCalculoDePensaoAlimenticia.liquidar` (subclasse de `Calculo` com verbas/FGTS por override; `RepoVerbaStub`) e lê `getValorDevido`; sem incidência sobre juros |
| GoldenGenIrpfGeracao.java | geração das ocorrências de IRPF na liquidação: escolha do regime pelo corte 28/07/2010 (caixa/competência), classificação das verbas em baldes (13º/férias/demais/anos-anteriores), tipos de tributação (em separado das férias, exclusiva do 13º, normal), contagem das competências do RRA + faixa × NM (início da faixa `((inicial−0,01)×NM)+0,01` piso 0), incidência sobre juros e deduções fixas (dependentes/aposentado, ×NM no RRA); + cenário C1 do **rateio das deduções** (prev/pensão/honorários) por balde no regime de caixa | dirige `MaquinaDeCalculoDeIrpf.liquidar` (subclasse de `Calculo` — verbas `Informada` via `RepoVerbaStub`, juros por override dos getters `getTotalDeJurosDaApuracaoDeJurosParaIrpf*`; C1 estuba `getPrevidenciaPrivada`/`getPensaoAlimenticiaDoCalculo`/`getValorTotalHonorariosDevidosPeloReclamante`/`calcularBrutoDevidoAoReclamante`) sobre uma `Irpf` com as flags; INSS acoplado (por competência) desligado; `RepositorioDeTabelaIrpf` estubado por JDBC; lê `irpf.getOcorrencias()` (tipo/base/alíquota/dedução/devido/competências/faixa; C1 lê o rateio prev+pensão+honorários por tipo) |
| GoldenGenLiquido.java | agregados de crédito do reclamante que formam o líquido: `calculaValorVerbaParaCreditoDoReclamante` (principal+juros+salário-família+seguro-desemprego+art.467) e `calculaValorFgtsParaCreditoDoReclamante` (FGTS+multa−depositado se deduzir) | subclasse de `Calculo` fixa os totais por override |
| GoldenGenBrutoComAcessorios.java | composição do bruto devido ao reclamante com os acessórios (FGTS que compõe o principal + multa + art. 467 − depositado se deduzir; salário-família; seguro-desemprego) | subclasse de `Calculo` fixa os totais por override (o método só acumula parcela a parcela); lê `calcularBrutoDevidoAoReclamante` |
| GoldenGenOrquestrador.java | caminho mínimo end-to-end: verbas → apuração de juros → multas (sobre o principal apurado) → bruto devido ao reclamante → honorários e custas (sobre o bruto) | combina os stubs de `GoldenGenApuracaoJuros` (reflection em `apurarJurosDasVerbas`) e `GoldenGenPenalidades` (Fgts/SalarioFamilia/SeguroDesemprego no-arg; repos de parâmetros de custas e teto RGPS); lê `calcularBrutoDevidoAoReclamante` e os totalizadores. `correcaoDasCustas=false` |
| GoldenGenApuracaoJuros.java | apuração de juros de mora sobre as verbas (agrupamento por competência + início dos juros, capital corrigido, taxa acumulada, juros por balde e totais) | dirige o método privado `Calculo.apurarJurosDasVerbas` via **reflection** sobre subclasse de `Calculo` (override `getVerbasAtivas`); verbas `Informada` com ocorrências à mão (`RepoVerbaStub` p/ `adicionarEmOcorrencias`); `ParametrosDeAtualizacao` com `JUROS_UM_PORCENTO` fixo e `BaseDeJurosDasVerbas=VERBAS` (default é VERBA_INSS!); incidências INSS/IRPF/prev. em false p/ evitar NPE. Saída `cenario;chave;valor` |
